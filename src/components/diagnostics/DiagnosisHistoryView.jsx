...
import { History, Filter, Trash2, Calendar, TrendingUp, Sparkles, AlertCircle, ArrowUpRight, Leaf } from "lucide-react";
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 text-xs font-bold bg-emerald-950/60 p-1.5 rounded-xl border border-emerald-900/60">
            ...
            >
              ...
              <option value="Corn" className="bg-slate-900 text-white">Maize / Corn</option>
              <option value="Soybean" className="bg-slate-900 text-white">Soybean</option>
              ...
              <option value="Onion" className="bg-slate-900 text-white">Onion</option>
              <option value="Grape" className="bg-slate-900 text-white">Grapes</option>
              <option value="Pepper" className="bg-slate-900 text-white">Bell Pepper</option>
      ...
      {sortedHistory.length > 1 && (
        <div className={`rounded-3xl p-6 border ${isDark ? "border-emerald-800/40 bg-[#072017]" : "border-slate-200 bg-white shadow-xl"}`}>
          ...
          <div className="h-32 flex items-end justify-between gap-2 pt-6 px-4 border-b border-emerald-900/40">
            {sortedHistory.slice(0, 8).reverse().map((item) => {
              const rawSev = typeof item.severity === "object" ? (item.severity?.tier || "Moderate") : (item.severity || "Moderate");
              ...
              const confVal = Math.round(item.confidence > 1 ? item.confidence : (item.confidence || 0) * 100);
              ...
              return (
                <div key={item.id} className="flex-1 flex flex-col items-center gap-2 group">
                  ...
                  <span className="text-[9px] font-mono text-slate-400 truncate max-w-[48px]">
                    {item.createdAt ? new Date(item.createdAt).toLocaleDateString(undefined, { month: "numeric", day: "numeric" }) : "Recent"}
                  </span>
                </div>
              );
            })}
      ...
      {loading ? (
        ...
        <div className={`rounded-3xl border p-12 text-center space-y-4 ${
          ...
          <h3 className={`text-lg font-black ${isDark ? "text-white" : "text-slate-900"}`}>No crop diagnoses recorded yet</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {sortedHistory.map((item) => {
            const confPercent = Math.round(item.confidence > 1 ? item.confidence : (item.confidence || 0) * 100);
            const cropName = typeof item.crop === "object" ? (item.crop?.name || "Crop") : (item.crop || item.cropType || "Crop");
            const conditionName = typeof item.condition === "object" ? (item.condition?.name || "Diagnosed Condition") : (item.condition || "Diagnosed Condition");
            const rawSev = typeof item.severity === "object" ? (item.severity?.tier || "Moderate") : (item.severity || "Moderate");
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="relative h-16 w-16 rounded-xl overflow-hidden bg-slate-950 shrink-0 border border-emerald-900/60 flex items-center justify-center">
                    {item.imageUrl ? (
                      <img src={item.imageUrl} alt={cropName} className="h-full w-full object-cover group-hover:scale-110 transition-transform duration-500" />
                    ) : (
                      <div className="h-full w-full flex flex-col items-center justify-center bg-emerald-950/40 text-emerald-400 p-1">
                        <Leaf size={22} />
                        <span className="text-[8px] font-bold uppercase truncate max-w-[56px] mt-0.5">{cropName}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black uppercase text-emerald-400">{cropName}</span>
                      <span className="text-[10px] font-mono text-slate-400 flex items-center gap-1">
                        <Calendar size={10} /> {new Date(item.createdAt || Date.now()).toLocaleDateString()}
                      </span>
                    </div>

                    <h4 className={`text-sm font-black truncate mt-0.5 ${isDark ? "text-white" : "text-slate-900"}`}>{conditionName}</h4>
