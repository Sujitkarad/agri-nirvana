...
Production checkpoints are authoritative. EfficientNetV2-L is the current
high-capacity production architecture; EfficientNetV2-S and legacy B0 remain
loadable only for controlled compatibility/testing. Crop filtering never
...
class EfficientNetDiseaseClassifier:
    def __init__(self, checkpoint_path: str):
        ...
        self.image_size = int(checkpoint.get("image_size", 448))
        ...
        if architecture == "efficientnet_v2_l":
            ...
            self.model_id = "agri-nirvana-efficientnet-v2-l"
        elif architecture == "efficientnet_v2_s":
            self.model = models.efficientnet_v2_s(weights=None)
            self.model_id = "agri-nirvana-efficientnet-v2-s-compat"
    ...
    def classify(self, pil_image: Image.Image, top_k: int = 5, crop_filter: Optional[str] = None) -> Dict:
        ...
        crop_match = not use_crop_filter or requested_crop == global_crop.lower() or crop_probability_mass >= 0.50
        ...
        predictions = [{"raw_label": raw, "crop": crop, "condition": condition, "confidence": round(float(prob), 4)} for prob, _, raw, crop, condition in top]
        best = predictions[0]
        return {
