...
def _split_single_train_three_way(root: Path, val_fraction: float = 0.15, test_fraction: float = 0.15, seed: int = 42) -> tuple[int, int, int]:
    ...
    for class_dir in sorted(train_root.iterdir()):
        ...
        for src in val_files:
            ...
            shutil.move(str(src), str(dest / src.name))
            val_total += 1
        train_total += len(files) - test_count - val_count
...
def _production_class_names() -> list[str]:
    ...
    if not names: raise RuntimeError("PlantVillage label feature has no class names")
...
def _field_class_match(crop: str, diagnosis: str, production_classes: list[str]) -> str | None:
    ...
    if not crop_text or not diagnosis_parts: return None
    ...
    for cls in production_classes:
        ...
        if disease_scores: candidates.append((0.65 * crop_score + 0.35 * max(disease_scores), cls))
