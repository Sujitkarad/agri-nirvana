"""Production crop diagnosis engine: one local model, fail closed."""
...
class ProductionInferenceEngine:
    ...
    def _safe(self, crop: str, status: str, reason: str, confidence: float = 0.0) -> Dict[str, Any]:
        return {"status": status, "is_valid_crop_image": status == "success", "crop": {"name": crop, "confidence_pct": round(confidence*100)}, "cropType": crop, "condition": "Uncertain Result" if status != "success" else "Analyzed Disease", "confidence": round(confidence,4), "confidence_pct": round(confidence*100), "severity": "Unknown", "severityPercentage": 0, "symptoms": [], "symptoms_observed": [], "differential_diagnoses": [], "uncertainty": {"abstain": status != "success", "reason": reason}, "recommendations": {"immediate": "Retake a clear close-up image in natural light.", "monitoring": "Do not apply disease-specific treatment from an uncertain result.", "prevention": "Continue normal field scouting.", "expert_help": "Consult a KVK/agriculture extension officer if symptoms spread."}, "modelName": self.model_name, "modelVersion": self.model_version, "modelSource": self.model_source, "isMock": status == "model_unavailable"}
    ...
    def analyze(self, pil_image: Image.Image, crop_type: str) -> Dict[str, Any]:
        ...
        d = classification.get("confidence_diagnostics", {})
        confidence = float(classification.get("confidence",0.0))
        mass = float(d.get("crop_probability_mass",0.0))
        if not bool(d.get("crop_match",False)) or mass < self.min_crop_mass: return self._safe(crop,"uncertain",f"Crop evidence is insufficient (mass={mass:.2f}).",confidence)
        ...
        if float(d.get("top2_margin",0.0)) < self.min_margin: return self._safe(crop,"uncertain","Top-2 diagnosis margin is too narrow.",confidence)
        if float(d.get("normalized_entropy",1.0)) > self.max_entropy: return self._safe(crop,"uncertain","Prediction entropy is too high.",confidence)
