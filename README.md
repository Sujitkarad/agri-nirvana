...
- Production architecture targets **EfficientNetV2-L**.
- Local trained checkpoints are required for the real production inference provider.
- Image-quality and plant-validation gates run before diagnosis.
- Crop compatibility is checked before accepting a result.
- Confidence threshold, top-2 margin, entropy, and crop-probability-mass gates are used to control uncertainty.
- Temperature-scaled calibration can be loaded from the trained checkpoint.
- Differential predictions and evidence are exposed for explainability.
- Low-confidence or unsupported cases abstain instead of being treated as confirmed diagnoses.
- Disease knowledge and severity estimation are kept separate from the vision classifier.
